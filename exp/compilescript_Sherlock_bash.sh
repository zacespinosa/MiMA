...
#
export PATH=$HDF5_DIR/bin:$CURL_DIR/bin:$ZLIB_DIR/bin:$NETCDF_DIR/bin:$NETCDF_FORTRAN_DIR/bin:$PATH
export INCLUDE=$HDF5_DIR/include:$CURL_DIR/include:$ZLIB_DIR/include:$NETCDF_DIR/include:$NETCDF_FORTRAN_DIR/include:$INCLUDE
export LD_LIBRARY_PATH=$HDF5_DIR/lib:$CURL_DIR/lib:$ZLIB_DIR/lib:$NETCDF_DIR/lib:$NETCDF_FORTRAN_DIR/lib:$LD_LIBRARY_PATH
export LIBRARY_PATH=$LD_LIBRARY_PATH
#
export MIMA_CONFIG_FFLAGS="`nc-config --fflags; nf-config --fflags` -I${HDF5_INC} -I${HDF5_LIB} -I${MPI_DIR}/include  -I${MPI_DIR}/lib -I${NETCDF_FORTRAN_DIR}/lib"
...
export MIMA_CONFIG_CFLAGS="`nc-config --cflags; nf-config --cflags` -I${MPI_DIR}/include -I${HDF5_INC} -I${ZLIB_DIR}/include -I${CURL_DIR}/include"
...
export MIMA_CONFIG_LDFLAGS=" -shared-intel -L/usr/local/lib `nc-config --libs; nc-config --flibs; nf-config --flibs` -L${MPI_DIR}/lib -L${HDF5_DIR}/lib -L${ZLIB_DIR}/lib -L${CURL_DIR}/lib"
...
#
echo "**"
echo "*** compile mppncombine step...:: ${mppnccombine}"
echo "compilers: CC=${CC}, FC=${FC}, LD=${LD}, MPIFC=${MPIFC}"
...
#    icc -O -o ${mppnccombine} -I${NETCDF_DIR}/include -I${NETCDF_FORTRAN_DIR}/include -I${MPI_DIR}/include -I{HDF5_DIR}/include -L${NETCDF_DIR}/lib -L${NETCDF_FORTRAN_DIR}/lib -L${MPI_DIR}/lib -L${HDF5_DIR}/lib -lnetcdf -lnetcdff  $cwd/../postprocessing/mppnccombine.c
    icc -O -o $mppnccombine -I${NETCDF_DIR}/include -I${NETCDF_FORTRAN_DIR}/include -L${NETCDF_DIR}/lib -L${NETCDF_FORTRAN_DIR}/lib $cwd/../postprocessing/mppnccombine.c -lnetcdf -lnetcdff
fi
# `nf-config --clibs; nc-config --libs; nf-config --cflags; nf-config --fflags`
#--------------------------------------------------------------------------------------------------------
...
if [ ! -d "${execdir}" ]; then
  ...
  echo " ** ** execdir ${execdir} already exists"
fi
#
if [ -e "${workdir}" ]; then
  ...
  echo "** ** working dir does not exist or should not contaminate run..."
fi
#--------------------------------------------------------------------------------------------------------
echo "**"
